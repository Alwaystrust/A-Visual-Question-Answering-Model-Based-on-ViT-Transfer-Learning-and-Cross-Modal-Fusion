...
class Encoder(nn.Module):
    ...
    def __init__(self, enc_method, input_size, hidden_size, out_size, method):
        '''
        input_size
        ...
        self.num_hid = hidden_size
        self.model_method = method
        
    def init_hidden(self, batch, x):
        # just to get the type of tensor
        device = x.device
        hid_shape = (self.nlayers * 1, batch, self.num_hid)
    ...
    def forward(self, inputs):
        if self.enc_method == 'cnn':
            ...
            batch = inputs.size(0)
            hidden = self.init_hidden(batch,inputs)
            self.rnn.flatten_parameters()
            
            out, _ = self.rnn(inputs,hidden)
            if self.model_method == 'ban':
                return out
            return out[:, -1]
        elif self.enc_method == 'transformer':
